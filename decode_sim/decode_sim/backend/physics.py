...
GOAL model — ball enters through the TOP opening:
  ...
  A ball SCORES when it crosses y = goal_height while descending (vy < 0)
  with x ∈ [goal_distance,  goal_distance + goal_depth].
"""
...
def simulate(p: ShotParams, dt: float = 0.001, t_max: float = 4.0) -> TrajectoryResult:
    ...
    while t < t_max:
        ...
        if not crossed_top and last_y > p.goal_height >= state[1]:
            ...
            res.made = (
                vy_at < 0
                and p.goal_distance <= x_cross <= p.goal_distance + p.goal_depth
            )
            crossed_top = True
...
def _simulate_made_batch(velocities, angles_deg,
    ...
    def _accel_np(cy, cvx, cvy):
        ...
        if enable_drag:
            ...
            ax_ = np.where(ok, -fd * cvx / spd / ARTIFACT_M, 0.0)
            ay_ = np.where(ok, -G - fd * cvy / spd / ARTIFACT_M, ay_)
        return ax_, ay_
    ...
    while t < t_max:
        ...
        # RK4 — y position at intermediate stages approximated to O(dt²)
        # (error ≈ 2e-5 m per step at dt=0.002, negligible for made/miss)
        ax1, ay1 = _accel_np(y,              vx,             vy            )
        ax2, ay2 = _accel_np(y + .5*dt*vy,   vx + .5*dt*ax1, vy + .5*dt*ay1)
        ax3, ay3 = _accel_np(y + .5*dt*vy,   vx + .5*dt*ax2, vy + .5*dt*ay2)
        ax4, ay4 = _accel_np(y + dt*vy,      vx + dt*ax3,    vy + dt*ay3   )

        nx  = x  + dt * vx + (dt * dt / 6.0) * (ax1 + ax2 + ax3)
        ny  = y  + dt * vy + (dt * dt / 6.0) * (ay1 + ay2 + ay3)
        ...
        if cross.any():
            ...
            in_goal = ((goal_distance <= x_cross) &
                       (x_cross <= goal_distance + goal_depth) &
                       (vy_at < 0))
            made   |= cross & in_goal
            done   |= cross
...
def find_optimal(goal_distance: float,
    ...
    for a_int in range(25, 81):    # 56 angles, 1° steps
        ...
        made = _simulate_made_batch(
            ...
            GOAL_TOP_HEIGHT_M, GOAL_DEPTH_M, enable_drag,
        )
